...
// track_event = <time:v> + [<midi_event> | <meta_event> | <sysex_event>]
{
	...
	fseek(mf->_fd,_startOffset+_currOffset,SEEK_SET);
...
//	fpos_t pos;
//	fgetpos(mf->_fd,&pos);
	uint32_t pos=ftell(mf->_fd);
  //#ifdef __MBED__
	_currOffset = pos - _startOffset;
 // #else
//	_currOffset = pos.__pos - _startOffset;
//  #endif
...
// ---------------------------- MIDI
	...
	case 0xe0 ... 0xef:
		...
		_mev.data[1] = fgetc(mf->_fd);
		_mev.data[2] = fgetc(mf->_fd);
	...
	case 0x00 ... 0x7f:	// MIDI run on message
	{
	  ...
	  {
		  _mev.data[i] = fgetc(mf->_fd);	// next byte
	  }	
...
//		printf("set tempo to %d %d\n",mf->getTickTime(),mf->getTempo());
	  ...
	  case 0x58:	// time signature
		mf->setTimeSignature(fgetc(mf->_fd), (1 << fgetc(mf->_fd)));	// denominator is 2^n
		fseek(mf->_fd,mLen-2,SEEK_CUR);
...
int MD_MFTrack::load(uint8_t trackId, MD_MIDIFile *mf)
{
  ...
  // track_chunk = "MTrk" + <length:4> + <track_event> [+ <track_event> ...]
  {
	...
	fgets(h, MTRK_HDR_SIZE+1,mf->_fd);
    h[MTRK_HDR_SIZE] = '\0';
...
//  #ifdef __MBED__
  _startOffset=pos;
//  #else
//  _startOffset=pos.__pos;
//  #endif
  _currOffset = 0;

  // Advance the file pointer to the start of the next track;
  if (fseek(mf->_fd,_startOffset+_length,SEEK_SET) == -1)
	  return(1);
