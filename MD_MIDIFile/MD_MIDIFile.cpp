...
#ifdef __MBED__
 uint64_t micros() {
    return (uint64_t)us_ticker_read() * 2;
}
#else
#include <sys/time.h>
 uint64_t micros() {
     ...
     return ((uint64_t)1000000 * tv.tv_sec + tv.tv_usec) / 4;     
}
...
void MD_MIDIFile::begin(char *psd)
{
	_sd = psd;
}
...
// Load the MIDI file into memory ready for processing
{
  ...
  _fd=fopen(_fileName, "rb");

  if(setvbuf(_fd,buf,_IOFBF,BUFSIZE)!=0) {
    return(1);
  }

/*  if(setvbuf(_fd,NULL,_IONBF,0)!=0) {
    return(1);    
  }
*/
